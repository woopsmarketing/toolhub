...
> **현재 상태:** 34개 툴 라이브 · **Phase 0~3.5 + 2.5~2.7 + 4.5 + 5 완료** · Phase 4 (본격 양산 34→90) + 도메인 결정/연결 진행 단계
...
### Phase 2 — 하네스/슬래시커맨드/서브에이전트 (2~3주) ✅ **완료** (2.1~2.7)
...
| 2.7 | 시범 툴 4개 동시 양산 (qr-code-generator / html-preview / savings-calculator / temperature-converter — form-to-visual·live-preview 등 다른 템플릿 검증 완료) | ✅ |
...
### Phase 4.5 — 전용 DB 분리 ✅ **완료** (2026-05-10)
> 결정: **Neon 대신 새 Supabase 프로젝트 사용** — 사용자 친숙도 + Phase 5 Auth 통합 효율 + §3 스키마 그대로 재사용.

| # | Task | 상태 |
|---|------|------|
| 4.5.1 | 전용 Supabase 프로젝트 생성 (`ceziiqfcciehvygufmqq`) + `.mcp.json` / `.env.local` / Vercel 환경변수 갱신 | ✅ |
| 4.5.2 | 기존 공유 프로젝트(seoworld)의 Toolhub 흔적 정리 (profiles 보존, 7 테이블 + 4 view DROP) | ✅ |
| 4.5.3 | 신규 프로젝트에 §3 스키마 적용 (8 테이블 + RLS + 인덱스 + 트리거 + 4 분석 view) | ✅ |
| 4.5.4 | 보안/성능 권장사항 일괄 정리 (view security_invoker, search_path 고정, RLS auth.uid() 최적화 등) — `20260510000000_harden_security_and_perf.sql` | ✅ |
| 4.5.5 | `src/lib/supabase/{client,server,types,index}.ts` 클라이언트 생성 (browser/server 분리) | ✅ |
| 4.5.6 | `trackToolEvent()` 에 익명 DB 전송 추가 (GA4 + Supabase 동시 fire-and-forget) | ✅ |
| 4.5.7 | CLAUDE.md §0/§1.1/§8 갱신 (Supabase 사용 정식화) | ✅ |
| 4.5.8 | (Phase 5 와 함께) 기존 LocalStorage 즐겨찾기 → DB 마이그레이션 헬퍼 | ⏳ Phase 5 |

### Phase 5 — 인증 + 클라우드 동기화 ✅ **완료** (2026-05-10)
| # | Task | 상태 |
|---|------|------|
| 5.1 | Supabase Auth + Google OAuth 콘솔 설정, `/auth/callback` route, `useUser` hook, `AuthButton` 컴포넌트, 헤더 통합 | ✅ |
| 5.2 | `useFavorite` dual-mode (LocalStorage ↔ DB) + `trackToolEvent` 에 user_id 자동 주입 | ✅ |
| 5.3 | 첫 로그인 시 LocalStorage → DB 자동 마이그레이션 (`syncLocalFavoritesToDb`, 멱등) | ✅ |
| 5.4 | 프로필 페이지 (`/[locale]/profile`) — 아바타/이메일/가입일/플랜/즐겨찾기, 닉네임 인라인 편집 | ✅ |
| 5.5 | 검증 (typecheck/lint/test/audit/build) + 보안/a11y sub-agent 점검 + 발견 사항 수정 + 문서 갱신 | ✅ |

#### Phase 5 검증 결과 요약
- code-review: Critical 1건 (analytics `as never`) 수정 완료
- security: High 1건 (callback 백슬래시 open redirect) 수정 완료, Medium 2건 (avatar img-src CSP / events INSERT CHECK 제약) 별도 follow-up
- a11y: High 2건 (input focus outline / focus ring opacity) 수정 완료, Medium 2건 (메뉴 키보드 + Cancel i18n) 부분 수정
- typecheck 0, lint 0 (신규 파일), test 177/177, audit 71/71, build 통과 (`/auth/callback` + `/[locale]/profile` 신규 route)
