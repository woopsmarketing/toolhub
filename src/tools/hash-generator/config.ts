...
export const config: ToolConfig = {
    ...
    ko: [
      {
        ...
        example: {
          input: "릴리스 노트의 SHA-256 값",
          output: "도구가 계산한 SHA-256 해시값과 비교하여 파일 변조 여부 확인",
        },
      },
      {
        ...
        example: {
          input: "AWS·토스·카카오 API 요청 body",
          output: "SHA-256 해시값 — 개발자 문서 예시와 대조 가능",
        },
      },
      {
        ...
        example: {
          input: "비트코인 블록 헤더의 일부 필드",
          output: "예상 해시값을 즉석에서 계산하여 확인",
        },
      },
    ],
    en: [
      {
        ...
        example: {
          input: "SHA-256 listed in a GitHub release",
          output: "Hash produced by this tool — match confirms file integrity",
        },
      },
      {
        ...
        example: {
          input: "AWS request body or Bitcoin block header field",
          output: "SHA-256 hash to cross-check against documentation samples",
        },
      },
    ],
  },
  ...
  guide: {
    ...
    en: {
      title: "About the Hash Generator",
      content:
    ...
    ko: [
      {
        ...
        a: "아니요. 이 해시 생성기는 모든 계산을 브라우저 안의 JavaScript로 수행하며, 네트워크로 어떤 데이터도 전송하지 않습니다. 민감한 개발용 문자열이나 내부 설정값도 안심하고 붙여넣을 수 있습니다.",
      },
      {
        q: "MD5와 SHA-1은 왜 ⚠️ 경고가 붙나요?",
        a: "MD5는 2004년 이후 실용적 충돌 공격이 알려져 있고 SHA-1도 2017년 SHAttered 공격으로 충돌이 공개되어, 현재 디지털 서명·TLS·암호학적 무결성 용도로는 권장되지 않습니다. 호환성 확인이나 비암호 체크섬용으로는 여전히 쓰이지만, 보안 목적이라면 SHA-256 이상을 사용해야 합니다.",
      },
      {
        q: "비밀번호를 SHA-256으로 저장해도 되나요?",
        a: "권장하지 않습니다. SHA 계열은 속도가 빨라 GPU로 초당 수십억 건의 추측이 가능하므로 비밀번호 저장에는 부적합합니다. 비밀번호는 bcrypt, argon2, scrypt처럼 작업 계수(work factor)를 조절할 수 있는 전용 해시 알고리즘으로 솔트(salt)와 함께 저장해야 합니다.",
      },
      {
        q: "같은 입력에서 왜 항상 같은 해시가 나오나요?",
        a: "해시 함수는 결정적(deterministic) 함수이므로 동일한 입력은 언제 어디서 실행하더라도 항상 같은 해시를 반환합니다. 반대로 해시값에서 원래 텍스트를 복원하는 것은 수학적으로 불가능에 가까우며, 레인보우 테이블을 통한 부분적 역추적만 제한적으로 가능합니다.",
      },
    ],
    en: [
      {
        q: "Is my input sent to a server?",
        a: "No. All hashing happens inside your browser using JavaScript, and no network request is made with your input. You can safely paste sensitive development strings or configuration snippets.",
      },
      {
        q: "Why are MD5 and SHA-1 marked with a warning?",
        a: "MD5 has had practical collision attacks since 2004, and SHA-1 was broken by the SHAttered attack in 2017. They are no longer recommended for digital signatures, TLS, or any cryptographic integrity use. They remain fine for legacy compatibility or non-security checksums, but for any security-sensitive purpose use SHA-256 or stronger.",
      },
      {
        q: "Can I use SHA-256 to store passwords?",
        a: "No. SHA-family hashes are designed to be fast, which means GPUs can guess billions of passwords per second. For password storage use a purpose-built algorithm such as bcrypt, argon2, or scrypt with a unique salt per user and a tunable work factor.",
      },
    ],
  },
