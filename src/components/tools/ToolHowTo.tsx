...
export default function ToolHowTo({ steps, title }: ToolHowToProps) {
  ...
  return (
    <section aria-labelledby="tool-howto-heading" className="mb-14">
      <div className="mb-5 flex items-center gap-2">
        <Lightbulb className="h-5 w-5 text-amber-500" aria-hidden="true" />
        <h2
          id="tool-howto-heading"
        ...
        <ol className="space-y-4">
          {steps.map((step, i) => (
            <li key={i} className="flex gap-4">
              <span
                aria-hidden="true"
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-bold text-white shadow-sm"
              >
                {i + 1}
              </span>
