...
## 입력
- 기술 설계: `.claude/pipeline/{{SLUG}}/03-architect.md` (slug, template, category 추출)
...
### 3단계: sync-loader 실행
`npm run sync-loader` 를 실행하여 ToolLoader.tsx를 자동 생성한다.
...
수정된 파일:
  ...
  ✔ src/components/tools/ToolLoader.tsx — sync-loader 자동 생성
...
## 파일 출력
보고를 `.claude/pipeline/{{SLUG}}/07-integration.md` 에 저장한다.

## 경계
- page.tsx를 절대 수정하지 않는다
- config.ts, logic.ts를 수정하지 않는다
- 기존 registry.ts 항목의 순서를 변경하지 않는다
- 새 템플릿 컴포넌트를 만들지 않는다
